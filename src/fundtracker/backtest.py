...
def diagnose_weights(
    ...
    """Find holdings whose weight looks wrong, from the pattern of our errors.

    If we carry too much of a holding, our estimate is too high exactly on the
    days that holding rose — so the error moves with its return. Regressing the
    daily error on each holding's daily return gives a slope that *is* the
    excess weight, in percentage points.

    This is what makes a stale holdings list diagnosable rather than merely
    suspected: a manager who halved a position leaves that signature behind.
    """
    estimated, actual = gather_series(fund, snapshot, days)
    frame = align_lag(estimated, actual, 0)
    if frame.empty:
        raise RuntimeError("Ingen overlappende dager å diagnostisere.")
    error = frame["error_pct"]
    ...
    for holding in snapshot.holdings:
        ...
        rate = fx[mapping.currency].pct_change() if mapping.currency in fx else 0.0
        nok = ((1 + local) * (1 + rate) - 1).reindex(error.index).dropna() * 100.0
        common = error.reindex(nok.index).dropna()
        if len(common) < 20 or nok.loc[common.index].var() == 0:
            continue
        series = nok.loc[common.index]
        slope = float(series.cov(common) / series.var())
        rows.append({
            "name": holding.name,
            "ticker": mapping.ticker,
            "weight_pct": holding.weight_pct,
            # Slope is a weight fraction; express it in the same unit as weights.
            "excess_weight_pct": slope * 100.0,
            "implied_weight_pct": holding.weight_pct - slope * 100.0,
            "correlation": float(series.corr(common)),
        })
    ...
    return (
        pd.DataFrame(rows)
        .assign(_rank=lambda d: d["excess_weight_pct"].abs())
...
def format_weight_diagnosis(frame: pd.DataFrame, top: int = 8) -> str:
    if frame.empty:
        return "Ikke nok data til å diagnostisere vektene."
    lines = [
        "Beholdninger der vekten ser feil ut, utledet av feilmønsteret",
        "",
        "  selskap                      ticker      vekt    for mye   antydet vekt",
    ]
    for _, r in frame.head(top).iterrows():
        lines.append(
            f"  {r['name'][:26]:<26}  {r['ticker']:<10} {r['weight_pct']:>6.2f} %"
            f"  {r['excess_weight_pct']:>+7.2f}   {r['implied_weight_pct']:>9.2f} %"
        )
    lines += [
        "",
        "  'For mye' er stigningstallet fra en regresjon av dagsfeilen på postens",
        "  egen dagsavkastning. Er det klart positivt, oppfører fondet seg som om",
        "  det eier mindre enn beholdningslista sier.",
    ]
    return "\n".join(lines)
